...
| RTSP/RTP input | experimental | `aimedia run` 已接 TCP interleaved 单路数据面：H.264/AAC-LC/G.711 直接进入有界 codec 队列，G.711 8kHz 单声道归一到 48kHz 双声道；MediaMTX 1.20.0 + FFmpeg 发布端已通过 180 秒 GPU 闭环、发布源 404 重连和 40ms RTT/20ms 抖动/1% 丢包短门禁，p95 90ms 且输出时间戳零回退。H.265 Main 软件源也已通过 90 秒 HEVC -> H.264/AAC TS/SRT GPU 门禁，覆盖重连和同等网络损伤，p95 107ms；UDP、非 48kHz 双声道 AAC、物理摄像机认证和两小时长稳未完成，不代表 GB28181 支持 |
...
| NVDEC HEVC input bridge | experimental | HEVC Main 8-bit 4:2:0 使用同一代际 NV12 surface 契约；SDK 13.0 release build、单个 1080p IRAP 解码和 90 秒 RTSP/HEVC -> NVDEC -> NVENC H.264 -> TS/SRT GPU 门禁均已在 RTX 5060 Laptop 通过。首帧 keyframe、时间戳、故障恢复、网络损伤和 surface 3/4 已验证；物理摄像机、两小时长稳、Main10/HDR/隔行/4:2:2/4:4:4 未覆盖 |
