...
| RTMP/RTMPS + FLV | foundation | V3-03A 已冻结 RTMP listener、RTMP/RTMPS publisher、传统 H.264/AAC FLV 子集、密钥引用、8 MiB 默认 message 上限和有界 pending 图；真实 `run` 仍返回 `rtmpDataPlanePending`，尚未实现 socket/FLV 数据面或平台互操作，不可用于直播 |
