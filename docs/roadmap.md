...
- [ ] **V3-03 RTMP/RTMPS 与 FLV**：接收编码器发布并向国内外平台推流；不建设观众
  ...
  - [ ] V3-03F：外部互操作、故障和长稳；全部通过后才升级为 `supported`。
    - [x] V3-03F1：FFmpeg RTMP publisher -> aimedia GPU -> aimedia RTMP publisher ->
      ...
      不计入 F4。
    - [x] V3-03F2：OBS 30.0.2.1 作为 RTMP publisher 与 consumer。发布链由 ffprobe
